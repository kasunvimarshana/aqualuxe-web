...
final class Bootstrap implements Singleton_Interface {
	...
	public function setup_theme_support(): void {
		...
		// Add editor styles (compiled editor.css)
		...
		// We render our own dark mode toggle in the header; disable module's footer toggle
		add_filter( 'aqualuxe_dark_mode_render_toggle', '__return_false' );
			...
			if ( $this->is_core_component_enabled( $component_key ) ) {
				try {
					if ( class_exists( $component_class ) ) {
						...
						$this->core_components[ $component_key ] = call_user_func( [ $component_class, 'get_instance' ] );
					}
				} catch ( \Exception $e ) {
