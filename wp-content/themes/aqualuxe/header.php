...
<body <?php body_class('bg-white dark:bg-slate-900 text-slate-800 dark:text-slate-100 min-h-screen flex flex-col'); ?> >
<?php wp_body_open(); ?>
...
<header class="border-b border-slate-200/60 dark:border-slate-700/60">
  <div class="container max-w-7xl mx-auto px-4 py-3 grid grid-cols-[auto,1fr,auto] items-center gap-4">
    ...
    <div class="flex items-center justify-end gap-2">
      <div class="hidden md:flex items-center gap-3">
        ...
        <?php elseif (class_exists('WooCommerce')): ?>
          ...
          <?php endif; ?>
        <?php endif; ?>
        <?php if (function_exists('shortcode_exists') && ! shortcode_exists('aqlx_search_toggle') && function_exists('Aqualuxe\\Inc\\load_modules')) { \Aqualuxe\Inc\load_modules(); } echo do_shortcode('[aqlx_search_toggle]'); ?>
        <?php if (function_exists('shortcode_exists') && ! shortcode_exists('aqlx_currency_switcher') && function_exists('Aqualuxe\\Inc\\load_modules')) { \Aqualuxe\Inc\load_modules(); } echo do_shortcode('[aqlx_currency_switcher]'); ?>
  <span class="text-sm leading-10">❤ <span class="aqlx-wishlist-count" aria-live="polite" aria-atomic="true" aria-label="<?php echo esc_attr__('Wishlist items count','aqualuxe'); ?>"><?php echo do_shortcode('[aqlx_wishlist_count]'); ?></span></span>
        <?php
      ...
      <button id="aqlx-mobile-toggle" class="md:hidden px-3 py-2 border rounded" aria-expanded="false" aria-controls="aqlx-mobile-menu" aria-label="<?php echo esc_attr__('Toggle navigation','aqualuxe'); ?>">☰</button>
    </div>
  </div>
