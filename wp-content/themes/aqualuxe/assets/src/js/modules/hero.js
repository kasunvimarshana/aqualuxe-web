...
(function(){
  ...
  let extAmbientBuf = null, extBubbleBuf = null;
  ...
  function playBubblePing(){
    try {
      ...
      if (extBubbleBuf){
        ...
        s.buffer = extBubbleBuf; s.connect(audioCtx.destination); s.start();
      } else {
  ...
  function enableAudio(){
    ...
    try {
      ...
      // Ambient: use external buffer if provided, otherwise synth noise
      const src = audioCtx.createBufferSource();
      if (extAmbientBuf){ src.buffer = extAmbientBuf; }
      else {
  ...
  gainNode.gain.linearRampToValueAtTime(volume, audioCtx.currentTime + 0.4);
      // Random bubble pings
      function bubble(){
        if (extBubbleBuf){ const s = audioCtx.createBufferSource(); s.buffer = extBubbleBuf; s.connect(audioCtx.destination); s.start(); return; }
        const o = audioCtx.createOscillator();
        const g = audioCtx.createGain();
  ...
  // Preload optional CC0 assets if provided via data attributes
  (async ()=>{
    try{
      const amb = audioBtn?.getAttribute('data-ambient-src') || '';
      const bub = audioBtn?.getAttribute('data-bubble-src') || '';
      if (amb){ if (!audioCtx) audioCtx = new (window.AudioContext||window.webkitAudioContext)(); const ab = await fetch(amb).then(r=>r.arrayBuffer()); extAmbientBuf = await audioCtx.decodeAudioData(ab); }
      if (bub){ if (!audioCtx) audioCtx = new (window.AudioContext||window.webkitAudioContext)(); const bb = await fetch(bub).then(r=>r.arrayBuffer()); extBubbleBuf = await audioCtx.decodeAudioData(bb); }
    }catch(e){}
  })();
  let idleTimer = null;
  ...
  if (volSlider) {
    ...
    // Keyboard +/- to adjust volume when slider focused
    volSlider.addEventListener('keydown', (e)=>{
      if (e.key === '+' || e.key === '=') { e.preventDefault(); const v = Math.min(100, (parseInt(volSlider.value,10)||0)+5); volSlider.value=String(v); volSlider.dispatchEvent(new Event('input')); }
      if (e.key === '-' || e.key === '_') { e.preventDefault(); const v = Math.max(0, (parseInt(volSlider.value,10)||0)-5); volSlider.value=String(v); volSlider.dispatchEvent(new Event('input')); }
    });
  ...
  // Animation loop
  let running = true;
  // Low power mode support: decimate updates and pause seaweed
  let lowPower = false;
  const powerBtn = document.getElementById('ax-power-toggle');
  // restore preference
  try { lowPower = localStorage.getItem('ax:lowpower') === 'on'; } catch(e){}
  // Inform other modules of current state
  try { window.dispatchEvent(new CustomEvent('ax:lowpower', { detail: { on: lowPower } })); } catch(e){}
  if (powerBtn){
    powerBtn.setAttribute('aria-pressed', lowPower ? 'true' : 'false');
    powerBtn.addEventListener('click', ()=>{
      lowPower = !lowPower;
      powerBtn.setAttribute('aria-pressed', lowPower ? 'true' : 'false');
      try{ localStorage.setItem('ax:lowpower', lowPower ? 'on' : 'off'); }catch(e){}
      try { window.dispatchEvent(new CustomEvent('ax:lowpower', { detail: { on: lowPower } })); } catch(e){}
    });
  }
  ...
  function animate(){
    ...
    // Seaweed sway
    if (!reduce && !lowPower && seaweedGroup.children.length){
    ...
    const skipFishUpdate = lowPower && (frameCount % 2 === 0); // update every other frame
    for (const f of fishes){
      // Tail wag
      if (!reduce && !lowPower) f.tail.rotation.z = Math.sin(t*8 + f.group.position.x)*0.5;
      ...
      if (!skipFishUpdate) f.dir.add(toPointer).normalize();
  ...
  f.group.position.addScaledVector(f.dir, (f.speed * boost) / (reduce?1.5:(lowPower?1.2:1)));
