...
class Module {
    ...
    public static function page(): void {
        ...
        // Advanced options
        echo '<div style="margin:10px 0;padding:10px;border:1px solid #e5e7eb;background:#fafafa">';
        echo '<h3 style="margin-top:0">' . esc_html__('Advanced options','aqualuxe') . '</h3>';
        echo '<p><label>' . esc_html__('Duplicate handling','aqualuxe') . ' <select name="dup_strategy">'
            .'<option value="skip">'.esc_html__('Skip','aqualuxe').'</option>'
            .'<option value="overwrite">'.esc_html__('Overwrite','aqualuxe').'</option>'
            .'<option value="merge">'.esc_html__('Merge','aqualuxe').'</option>'
            .'</select></label></p>';
        echo '<p><label>' . esc_html__('Date range start','aqualuxe') . ' <input type="date" name="filters[start_date]"></label> '
            .'<label style="margin-left:1rem">' . esc_html__('end','aqualuxe') . ' <input type="date" name="filters[end_date]"></label></p>';
        echo '<p><label>' . esc_html__('Include categories (comma names)','aqualuxe') . ' <input type="text" name="filters[include_cats]" placeholder="Blog, News"></label> '
            .'<label style="margin-left:1rem">' . esc_html__('Exclude categories','aqualuxe') . ' <input type="text" name="filters[exclude_cats]" placeholder="Old"></label></p>';
        echo '<p><label>' . esc_html__('Randomization level (0-100)','aqualuxe') . ' <input type="number" min="0" max="100" name="randomization" value="50" style="width:70px"></label>'
            .' <label style="margin-left:1rem">' . esc_html__('Locale','aqualuxe') . ' <input type="text" name="locale" value="en_US" size="8"></label>'
            .' <label style="margin-left:1rem">' . esc_html__('Seed','aqualuxe') . ' <input type="number" name="seed" placeholder="optional" style="width:120px"></label>'
            .' <label style="margin-left:1rem"><input type="checkbox" name="use_txn" value="1"> ' . esc_html__('Use DB transactions (beta)','aqualuxe') . '</label></p>';
        echo '</div>';
        ...
        echo '<div id="aqlx-demo-plan" style="display:none;max-height:240px;overflow:auto;border:1px solid #e5e7eb;padding:8px;margin:8px 0"></div>';
        echo '<input type="hidden" name="plan_json" id="aqlx-plan-json" value="" />';
        echo '</fieldset>';
        echo '</form>';
      ...
      $js = "(function(){\n" .
          ...
          "const planBox=document.getElementById('aqlx-demo-plan');\n" .
          "const planField=document.getElementById('aqlx-plan-json');\n" .
          ...
          "if(previewBtn&&form){previewBtn.addEventListener('click',async function(){const fd=new FormData(form);fd.set('action','aqlx_demo_preview');fd.append('nonce',".$nonce_js.");try{const r=await fetch(".$ajax_js.",{method:'POST',body:fd,credentials:'same-origin'});const d=await r.json();if(d&&d.success){append('--- Preview ---');append(d.data.summary);if(planBox&&d.data.plan){planBox.style.display='';planBox.innerHTML='';Object.keys(d.data.plan).forEach(sec=>{const list=d.data.plan[sec];if(!Array.isArray(list)||!list.length)return;const h=document.createElement('div');h.style.fontWeight='bold';h.style.marginTop='6px';h.textContent=sec;planBox.appendChild(h);list.forEach((item,idx)=>{const id='aqlx-plan-'+sec+'-'+idx;const wrap=document.createElement('label');wrap.style.display='block';wrap.innerHTML='<input type=\"checkbox\" data-sec=\"'+sec+'\" data-idx=\"'+idx+'\" checked> '+(item.title||('Item '+(idx+1)));planBox.appendChild(wrap);});});const collect=()=>{const chosen={};planBox.querySelectorAll('input[type=checkbox]').forEach(cb=>{const s=cb.getAttribute('data-sec');const i=parseInt(cb.getAttribute('data-idx'));if(!chosen[s])chosen[s]=[];if(cb.checked)chosen[s].push(i);});planField.value=JSON.stringify({indices:chosen,source:d.data.plan});};planBox.addEventListener('change',collect);collect();}append('--- End Preview ---');}else{append('Preview failed');}}catch(e){append('Preview error: '+e.message);}});}\n" .
    ...
    $cfg = isset($_POST['cfg']) && is_array($_POST['cfg']) ? array_map('sanitize_text_field', wp_unslash($_POST['cfg'])) : [];
        ...
        // Advanced
        $dup = in_array(($_POST['dup_strategy'] ?? 'skip'), ['skip','overwrite','merge'], true) ? $_POST['dup_strategy'] : 'skip';
        $filters = [
            'start_date' => sanitize_text_field($_POST['filters']['start_date'] ?? ''),
            'end_date'   => sanitize_text_field($_POST['filters']['end_date'] ?? ''),
            'include_cats' => sanitize_text_field($_POST['filters']['include_cats'] ?? ''),
            'exclude_cats' => sanitize_text_field($_POST['filters']['exclude_cats'] ?? ''),
        ];
        $randomization = min(100, max(0, (int)($_POST['randomization'] ?? 50)));
        $locale = sanitize_text_field($_POST['locale'] ?? 'en_US');
        $seed = isset($_POST['seed']) && $_POST['seed'] !== '' ? (int)$_POST['seed'] : wp_rand();
        $use_txn = !empty($_POST['use_txn']);
        $plan_json = isset($_POST['plan_json']) ? json_decode(wp_unslash((string)$_POST['plan_json']), true) : null;
    ...
    $s = self::get_state();
    $tail = array_slice(($s['logs'] ?? []), -10);
    wp_send_json_success(['status'=>$s['status'] ?? 'idle', 'progress'=>(int)($s['progress'] ?? 0), 'message'=>$s['last_message'] ?? '', 'logs'=>$tail]);
    }
    ...
    private static function step_create_media(array &$state): string {
        ...
        for ($i=0; $i<$count; $i++) {
            ...
            if ($attach_id) {
                $state['created']['media'][] = $attach_id;
                update_post_meta($attach_id, self::META_FLAG, 1);
                update_post_meta($attach_id, '_wp_attachment_image_alt', 'AquaLuxe placeholder #'.$i);
                $created++;
            }
        }
    ...
    private static function step_create_tax(array &$state): string {
        ...
        $cats = [
            ['name'=>'Freshwater','children'=>[['name'=>'Shrimp'],['name'=>'Plants']]],
            ['name'=>'Saltwater','children'=>[['name'=>'Reef'],['name'=>'Fish']]],
            ['name'=>'Equipment','children'=>[['name'=>'Filtration'],['name'=>'Lighting']]],
        ];
        foreach ($cats as $c) {
            $term = wp_insert_term($c['name'], 'category');
            if (is_wp_error($term)) continue; $tid = (int)$term['term_id']; add_term_meta($tid, self::META_FLAG, 1, true); $state['created']['terms'][] = $tid;
            foreach ($c['children'] as $child) { $ct = wp_insert_term($child['name'], 'category', ['parent'=>$tid]); if (!is_wp_error($ct)) { add_term_meta($ct['term_id'], self::META_FLAG, 1, true); $state['created']['terms'][] = (int)$ct['term_id']; } }
        }
        // Service categories
        foreach ([['name'=>'Installation'],['name'=>'Maintenance'],['name'=>'Consulting','children'=>[['name'=>'Remote'],['name'=>'Onsite']]]] as $c) {
            $term = wp_insert_term($c['name'], 'service_cat'); if (is_wp_error($term)) continue; $tid = (int)$term['term_id']; add_term_meta($tid, self::META_FLAG, 1, true); $state['created']['terms'][] = $tid;
            if (!empty($c['children'])) foreach ($c['children'] as $child) { $ct = wp_insert_term($child['name'], 'service_cat', ['parent'=>$tid]); if (!is_wp_error($ct)) { add_term_meta($ct['term_id'], self::META_FLAG, 1, true); $state['created']['terms'][] = (int)$ct['term_id']; } }
        }
    ...
    private static function step_create_products_simple(array &$state): string {
        ...
        for ($i=1; $i<=$count; $i++) {
            $id = wp_insert_post([
                ...
                'post_status' => 'publish',
                'post_type' => 'product',
                'meta_input' => [ self::META_FLAG => 1, '_price' => rand(10,200), '_regular_price'=> rand(10,200), '_sku'=>'AQLX-S-'.str_pad((string)$i,5,'0',STR_PAD_LEFT), '_manage_stock'=>'yes', '_stock'=>rand(5,50) ],
            ]);
    ...
    private static function step_create_products_variable(array &$state): string {
        if (!class_exists('WooCommerce')) return __('Skipped (WooCommerce inactive)','aqualuxe');
        $sizes = ['S','M','L']; $colors=['Red','Blue','Green']; $materials=['Glass','Acrylic'];
        ...
        for ($k=1; $k<=$n; $k++) {
            ...
            // Attributes (product-level, not global taxonomies)
            $attr_size = [ 'name' => 'Size', 'value' => implode(' | ', $sizes), 'position'=>0, 'is_visible'=>1, 'is_variation'=>1, 'is_taxonomy'=>0 ];
            $attr_color = [ 'name' => 'Color', 'value' => implode(' | ', $colors), 'position'=>1, 'is_visible'=>1, 'is_variation'=>1, 'is_taxonomy'=>0 ];
            $attr_material = [ 'name' => 'Material', 'value' => implode(' | ', $materials), 'position'=>2, 'is_visible'=>1, 'is_variation'=>1, 'is_taxonomy'=>0 ];
            update_post_meta($product_id, '_product_attributes', [ 'size' => $attr_size, 'color' => $attr_color, 'material'=>$attr_material ]);
            ...
            foreach ($sizes as $s) {
                foreach ($colors as $c) {
                    ...
                    $variation_id = wp_insert_post([
                        ...
                        'post_name' => 'variation-' . $product_id . '-' . sanitize_title($s.'-'.$c.'-'.$m),
                        'post_status' => 'publish',
                    ...
                    if ($variation_id) {
                        ...
                        update_post_meta($variation_id, 'attribute_material', $m);
                        $price = rand(10,200);
                        update_post_meta($variation_id, '_regular_price', $price);
                        update_post_meta($variation_id, '_price', $price);
                        update_post_meta($variation_id, '_manage_stock', 'yes');
                        update_post_meta($variation_id, '_stock', rand(1,25));
                    }
                }
            }
            }
