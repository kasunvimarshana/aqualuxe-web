...
function aqlx_diag_page(){
    ...
    $checks['css_exists'] = $css && file_exists(AQUALUXE_PATH . 'assets/dist' . $css);
    $checks['js_exists'] = $js && file_exists(AQUALUXE_PATH . 'assets/dist' . $js);
    // CSP non-empty
    $csp = function_exists('aqualuxe_default_csp') ? aqualuxe_default_csp() : '';
    $csp = apply_filters('aqualuxe/security_csp', $csp);
    ...
    if (empty($manifest)) {
        ...
        foreach ($manifest as $k=>$v){
            ...
            $exists = file_exists(AQUALUXE_PATH . 'assets/dist' . $san) ? 'yes' : 'no';
    ...
    if (is_array($manifest)) {
        foreach ($manifest as $k=>$v) {
            ...
            $data['manifest'][] = [
                ...
                'value' => $v,
                'sanitized' => $san,
                'exists' => file_exists(AQUALUXE_PATH . 'assets/dist' . $san),
                'url' => AQUALUXE_URI . 'assets/dist' . $san,
            ];
        }
    }
