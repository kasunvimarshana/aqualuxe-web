...
if (is_admin()) {
    ...
    function aqualuxe_importer_page(){
        ...
        (function(){
            ...
            function run(action){
                ...
                // entity selections
                data.append('imp_pages', $('#imp_pages').checked ? '1' : '0');
                data.append('imp_products', $('#imp_products').checked ? '1' : '0');
                data.append('imp_posts', $('#imp_posts').checked ? '1' : '0');
                data.append('imp_services', $('#imp_services').checked ? '1' : '0');
                data.append('imp_events', $('#imp_events').checked ? '1' : '0');
    ...
    add_action('wp_ajax_aqualuxe_import_step', function(){
        ...
        $opts = [
            'pages' => !empty($_POST['imp_pages']),
            'products' => !empty($_POST['imp_products']),
            'posts' => !empty($_POST['imp_posts']),
            'services' => !empty($_POST['imp_services']),
            'events' => !empty($_POST['imp_events']),
        ];
        try {
