...
1. Install dependencies (inside theme folder):
   - npm install
2. Build assets:
   - npm run build (Laravel Mix) or:
   - npm run build:wp (Webpack)
   - For development: npm run dev or npm run dev:wp
...
## Demo Importer
- Admin → Aqualuxe Importer → Start Import
- Runs idempotent steps: prepare → pages → taxonomies → listings → stores → menu → WooCommerce (if active) → finish
- Safe to re-run; only ensures missing content.
