...
function aqualuxe_get_schema_markup() {
    ...
    // Home
    if (is_front_page()) {
        $schema['html']['itemtype'] = 'https://schema.org/WebSite';
    }
    
    // Blog
    elseif (is_home() || is_category() || is_tag() || is_date() || is_author()) {
        $schema['html']['itemtype'] = 'https://schema.org/Blog';
    }
    
    // Single post
    elseif (is_singular('post')) {
        $schema['html']['itemtype'] = 'https://schema.org/BlogPosting';
    ...
    // Page
    elseif (is_page()) {
        $schema['html']['itemtype'] = 'https://schema.org/WebPage';
    }
    
    // Search
    elseif (is_search()) {
        $schema['html']['itemtype'] = 'https://schema.org/SearchResultsPage';
    }
    ...
    elseif (aqualuxe_is_woocommerce_active()) {
        ...
        if (is_shop() || is_product_category() || is_product_tag()) {
            $schema['html']['itemtype'] = 'https://schema.org/CollectionPage';
        }
        
        // Product
        elseif (is_product()) {
            $schema['html']['itemtype'] = 'https://schema.org/ItemPage';
        }
    }
 ...
 * @return array
 */
function aqualuxe_get_open_graph_metadata() {
    $metadata = [
    ...
    if (is_front_page()) {
        ...
        $metadata['og:title'] = get_bloginfo('name');
        $metadata['og:description'] = get_bloginfo('description');
        $metadata['og:url'] = home_url('/');
    }
    
    // Blog
    elseif (is_home()) {
        $metadata['og:type'] = 'website';
        $metadata['og:title'] = get_the_title(get_option('page_for_posts'));
        $metadata['og:url'] = get_permalink(get_option('page_for_posts'));
    }
    ...
    elseif (is_singular('post')) {
        ...
        $metadata['og:description'] = aqualuxe_get_excerpt(55);
        $metadata['og:url'] = get_permalink();
        
        if (has_post_thumbnail()) {
            $metadata['og:image'] = aqualuxe_get_image_url(get_post_thumbnail_id(), 'large');
            $metadata['og:image:alt'] = aqualuxe_get_image_alt(get_post_thumbnail_id());
        }
        
        $metadata['article:published_time'] = get_the_date('c');
        $metadata['article:modified_time'] = get_the_modified_date('c');
        
        // Author
        $author_id = get_the_author_meta('ID');
        $metadata['article:author'] = get_author_posts_url($author_id);
        ...
        if ($categories) {
            foreach ($categories as $category) {
                $metadata['article:section'] = $category->name;
                break;
            }
        }
    ...
    elseif (is_category()) {
        ...
        $metadata['og:type'] = 'website';
        $metadata['og:title'] = $term->name;
        $metadata['og:description'] = $term->description;
        $metadata['og:url'] = get_term_link($term);
    }
    ...
    elseif (is_author()) {
        ...
        $metadata['og:type'] = 'profile';
        $metadata['og:title'] = $author->display_name;
        $metadata['og:url'] = get_author_posts_url($author->ID);
        $metadata['profile:first_name'] = get_the_author_meta('first_name', $author->ID);
        $metadata['profile:last_name'] = get_the_author_meta('last_name', $author->ID);
    }
    
    // Search
    elseif (is_search()) {
        $metadata['og:type'] = 'website';
        $metadata['og:title'] = sprintf(__('Search Results for: %s', 'aqualuxe'), get_search_query());
        $metadata['og:url'] = get_search_link();
    }
    
    // 404
    elseif (is_404()) {
        $metadata['og:type'] = 'website';
        $metadata['og:title'] = __('Page Not Found', 'aqualuxe');
        $metadata['og:url'] = home_url('404');
    }
    ...
    elseif (aqualuxe_is_woocommerce_active()) {
        ...
        if (is_shop()) {
            $metadata['og:type'] = 'website';
            $metadata['og:title'] = woocommerce_page_title(false);
            $metadata['og:url'] = get_permalink(wc_get_page_id('shop'));
        }
        ...
        elseif (is_product_category()) {
            ...
            if ($thumbnail_id) {
                $metadata['og:image'] = aqualuxe_get_image_url($thumbnail_id, 'large');
                $metadata['og:image:alt'] = $term->name;
            }
        }
        ...
        elseif (is_product()) {
            ...
            if ($attachment_ids) {
                foreach ($attachment_ids as $attachment_id) {
                    $metadata['og:image'][] = aqualuxe_get_image_url($attachment_id, 'large');
                }
            }
            
            // Price
            if ($product->get_price()) {
                $metadata['product:price:amount'] = $product->get_price();
                $metadata['product:price:currency'] = get_woocommerce_currency();
            }
            
            // Availability
            $metadata['product:availability'] = $product->is_in_stock() ? 'in stock' : 'out of stock';
        }
    }
 ...
 * @return array
 */
function aqualuxe_get_twitter_card_metadata() {
    $metadata = [
    ...
    if ($twitter_username) {
        $metadata['twitter:site'] = '@' . str_replace('@', '', $twitter_username);
    }
    
    // Home
    if (is_front_page()) {
        $metadata['twitter:title'] = get_bloginfo('name');
        $metadata['twitter:description'] = get_bloginfo('description');
    }
    
    // Blog
    elseif (is_home()) {
        $metadata['twitter:title'] = get_the_title(get_option('page_for_posts'));
    }
    
    // Single post
    elseif (is_singular('post')) {
        $metadata['twitter:title'] = get_the_title();
        $metadata['twitter:description'] = aqualuxe_get_excerpt(55);
        
        if (has_post_thumbnail()) {
            $metadata['twitter:image'] = aqualuxe_get_image_url(get_post_thumbnail_id(), 'large');
            $metadata['twitter:image:alt'] = aqualuxe_get_image_alt(get_post_thumbnail_id());
        }
        
        // Author
        $author_id = get_the_author_meta('ID');
        $twitter_author = get_the_author_meta('twitter', $author_id);
        
        if ($twitter_author) {
            $metadata['twitter:creator'] = '@' . str_replace('@', '', $twitter_author);
        }
    }
    ...
    elseif (is_category()) {
        ...
        $metadata['twitter:title'] = $term->name;
        $metadata['twitter:description'] = $term->description;
    }
    ...
    elseif (is_author()) {
        ...
        $metadata['twitter:title'] = $author->display_name;
    }
    
    // Search
    elseif (is_search()) {
        $metadata['twitter:title'] = sprintf(__('Search Results for: %s', 'aqualuxe'), get_search_query());
    }
    
    // 404
    elseif (is_404()) {
        $metadata['twitter:title'] = __('Page Not Found', 'aqualuxe');
    }
    ...
    elseif (aqualuxe_is_woocommerce_active()) {
        // Shop
        if (is_shop()) {
            $metadata['twitter:title'] = woocommerce_page_title(false);
        }
        ...
        elseif (is_product_category()) {
            ...
            if ($thumbnail_id) {
                $metadata['twitter:image'] = aqualuxe_get_image_url($thumbnail_id, 'large');
                $metadata['twitter:image:alt'] = $term->name;
            }
        }
