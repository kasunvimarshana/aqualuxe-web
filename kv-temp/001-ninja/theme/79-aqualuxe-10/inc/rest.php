...
if (function_exists('add_action')) {
    add_action('rest_api_init', static function (): void {
        ...
        // Analytics endpoint: accepts lightweight client-side events (e.g., CTA clicks).
        register_rest_route('aqlx/v1', '/analytics', [
            'methods' => 'POST',
            'permission_callback' => '__return_true',
            'callback' => static function (\WP_REST_Request $request) {
                $data = $request->get_json_params();
                if (! is_array($data)) { $data = []; }
                $event = isset($data['event']) && is_array($data['event']) ? $data['event'] : [];
                // Minimal sanitization
                $name  = isset($event['name']) ? sanitize_key((string) $event['name']) : 'event';
                $type  = isset($event['type']) ? sanitize_key((string) $event['type']) : 'cta_click';
                $ts    = isset($event['ts']) ? (int) $event['ts'] : time();

                // Buffer in a transient (simple stub). A production site would stream to a proper store.
                $buf_key = 'aqlx_analytics_buffer';
                $buf = get_transient($buf_key);
                if (! is_array($buf)) { $buf = []; }
                $buf[] = [ 'type' => $type, 'name' => $name, 'ts' => $ts, 'ip' => $_SERVER['REMOTE_ADDR'] ?? '' ];
                if (count($buf) > 200) { $buf = array_slice($buf, -200); }
                set_transient($buf_key, $buf, 10 * MINUTE_IN_SECONDS);
