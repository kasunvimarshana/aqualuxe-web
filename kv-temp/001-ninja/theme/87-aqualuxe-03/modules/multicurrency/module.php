...
if (!\function_exists('shortcode_exists') || !\call_user_func('shortcode_exists', 'aqualuxe_currency_switcher')) {
    \add_shortcode('aqualuxe_currency_switcher', function(){
        $allowed = \function_exists('apply_filters') ? \call_user_func('apply_filters', 'aqualuxe/currency/allowed', ['USD','EUR','GBP']) : ['USD','EUR','GBP'];
        $current = \function_exists('apply_filters') ? \call_user_func('apply_filters', 'aqualuxe/currency/current', 'USD') : 'USD';
        $action = '#';
        if (\function_exists('add_query_arg')) {
            $actionUrl = \call_user_func('add_query_arg', []);
            $action = \function_exists('esc_url') ? \call_user_func('esc_url', $actionUrl) : $actionUrl;
        }
        $label = \function_exists('esc_attr__') ? \call_user_func('esc_attr__', 'Currency selector','aqualuxe') : 'Currency selector';
        $labelSelect = \function_exists('esc_html__') ? \call_user_func('esc_html__', 'Select currency','aqualuxe') : 'Select currency';
        $out = '<form method="get" action="' . $action . '" class="aqlx-currency" aria-label="' . $label . '">';
        $out .= '<label class="sr-only" for="aqlx-currency-select">' . $labelSelect . '</label>';
        $out .= '<select id="aqlx-currency-select" name="currency" onchange="this.form.submit()">';
        foreach ($allowed as $c) {
            $sel = (\function_exists('selected') ? \call_user_func('selected', $current, $c, false) : ($current === $c ? 'selected' : ''));
            $val = \function_exists('esc_attr') ? \call_user_func('esc_attr', $c) : $c;
            $txt = \function_exists('esc_html') ? \call_user_func('esc_html', $c) : $c;
            $out .= '<option value="' . $val . '" ' . $sel . '>' . $txt . '</option>';
        }
        $out .= '</select>';
        ...
        foreach ($_GET as $k=>$v) {
            ...
            $key = \function_exists('esc_attr') ? \call_user_func('esc_attr', (string)$k) : (string)$k;
            $val = \function_exists('esc_attr') ? \call_user_func('esc_attr', (\function_exists('wp_unslash') ? \call_user_func('wp_unslash', (string)$v) : (string)$v)) : (string)$v;
            $out .= '<input type="hidden" name="' . $key . '" value="' . $val . '" />';
        }
        $updateTxt = \function_exists('esc_html__') ? \call_user_func('esc_html__', 'Update','aqualuxe') : 'Update';
        $out .= '<noscript><button type="submit" class="button">' . $updateTxt . '</button></noscript>';
        $out .= '</form>';
        return $out;
    });
}
