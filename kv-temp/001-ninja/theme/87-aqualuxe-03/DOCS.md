...
## Quick Start
- Requirements: WordPress >= 6.0, PHP >= 8.0, Node 20 LTS
...
## Testing & CI
- CI workflow builds and PHP‑lints the theme on push/PR to `dev`/`main`/`dev-*`. Uses Node 20 with npm cache.
...
## Shortcodes
- [aqualuxe_currency_switcher] – Provided by the Multicurrency module; renders only when that module is enabled.
- [aqualuxe_language_switcher] – Provided by the Multilingual module; uses Polylang/WPML when present, falls back to a locale badge.
- [aqualuxe_subscribe_form] – Newsletter capture (stores entries under Submissions CPT).
- [aqualuxe_contact] – Map + contact form. Example: `[aqualuxe_contact address="AquaLuxe HQ, Colombo"]`.
