...
class AquaLuxe_Events_Module {
    ...
    public function enqueue_assets() {
        // Only load for event-related pages
        if (is_singular('aqualuxe_event') || is_post_type_archive('aqualuxe_event') || $this->has_event_shortcode()) {
            // Add event-specific variables to main script
            wp_localize_script('aqualuxe-script', 'aqualuxe_events_vars', [
            ...
            $custom_css = '
                .event-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(350px, 1fr)); gap: 2rem; }
                .event-card { background: white; border-radius: 0.75rem; overflow: hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.1); transition: transform 0.2s; }
                .event-card:hover { transform: translateY(-4px); }
                .event-meta { display: flex; align-items: center; gap: 1rem; font-size: 0.875rem; color: #6b7280; }
                .event-registration-form { background: #f8fafc; padding: 2rem; border-radius: 0.5rem; }
                .event-calendar { background: white; border-radius: 0.5rem; padding: 1.5rem; }
                .event-date { background: #3b82f6; color: white; padding: 0.5rem 1rem; border-radius: 0.25rem; font-weight: 600; }
                .event-sold-out { background: #ef4444; color: white; padding: 0.25rem 0.75rem; border-radius: 0.25rem; font-size: 0.75rem; }
            ';
    ...
    /**
     * Check if current page has event shortcode
     */
    private function has_event_shortcode() {
        ...
        return (
            has_shortcode($post->post_content, 'aqualuxe_events') || 
