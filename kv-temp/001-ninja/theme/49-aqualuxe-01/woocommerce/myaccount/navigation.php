...
<nav class="woocommerce-MyAccount-navigation bg-white dark:bg-gray-800 rounded-lg shadow-sm overflow-hidden mb-6">
	<div class="account-user p-6 border-b border-gray-200 dark:border-gray-700">
		<div class="flex items-center">
			<div class="account-avatar mr-4">
				<?php echo get_avatar( get_current_user_id(), 64, '', '', array( 'class' => 'rounded-full' ) ); ?>
			</div>
			<div class="account-user-info">
				<p class="account-user-name font-medium text-lg"><?php echo esc_html( wp_get_current_user()->display_name ); ?></p>
				<p class="account-user-email text-sm text-gray-600 dark:text-gray-400"><?php echo esc_html( wp_get_current_user()->user_email ); ?></p>
			</div>
		</div>
	</div>
	...
	<ul class="navigation-links">
		<?php foreach ( wc_get_account_menu_items() as $endpoint => $label ) : ?>
			<li class="<?php echo wc_get_account_menu_item_classes( $endpoint ); ?> border-b border-gray-200 dark:border-gray-700 last:border-b-0">
				<a href="<?php echo esc_url( wc_get_account_endpoint_url( $endpoint ) ); ?>" class="block p-4 hover:bg-gray-50 dark:hover:bg-gray-700 transition-colors duration-200 <?php echo is_wc_endpoint_url( $endpoint ) ? 'bg-gray-50 dark:bg-gray-700 text-primary-600 dark:text-primary-400 font-medium' : ''; ?>">
					<?php
					...
					switch ( $endpoint ) {
						...
						case 'edit-account':
							...
							break;
						case 'wishlist':
							$icon = '<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>';
							break;
