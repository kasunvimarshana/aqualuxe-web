...
<?php
	...
	public function author_select( $n, $author ) {
		...
		echo ' ' . wp_dropdown_users( array( 'name' => "user_map[$n]", 'id' => 'imported_authors_' . esc_attr( $n ), 'multi' => true, 'show_option_all' => __( '- Select -', 'wordpress-importer' ), 'show' => 'display_name_with_login', 'echo' => 0 ) );
	...
	public function process_categories() {
		...
		foreach ( $this->categories as $cat ) {
			...
			if ( is_array( $id ) && isset( $id['term_id'] ) ) {
				$term_id = $id['term_id'];
				if ( isset( $cat['term_id'] ) ) {
					$this->processed_terms[ intval( $cat['term_id'] ) ] = $term_id;
				}
				$this->process_termmeta( $cat, $term_id );
			}
		}
	...
	public function process_tags() {
		...
		foreach ( $this->tags as $tag ) {
			...
			if ( isset( $tag['term_id'] ) ) {
				$this->processed_terms[ intval( $tag['term_id'] ) ] = $term_id;
			}

			$this->process_termmeta( $tag, $term_id );
		}
	...
	public function process_terms() {
		...
		foreach ( $this->terms as $term ) {
			...
			if ( is_array( $id ) && isset( $id['term_id'] ) ) {
				$term_id = $id['term_id'];
				if ( isset( $term['term_id'] ) ) {
					$this->processed_terms[ intval( $term['term_id'] ) ] = $term_id;
				}
	...
	public function process_posts() {
		...
		foreach ( $this->posts as $post ) {
			...
			if ( ! post_type_exists( $post['post_type'] ) ) {
				printf( esc_html__( 'Failed to import &#8220;%1$s&#8221;: Invalid post type %2$s', 'wordpress-importer' ), esc_html($post['post_title']), esc_html($post['post_type']) );
				echo '<br />';
			...
			if ( $post_exists && get_post_type( $post_exists ) == $post['post_type'] ) {
				printf( esc_html__('%1$s &#8220;%2$s&#8221; already exists.', 'wordpress-importer'), $post_type_object->labels->singular_name, esc_html($post['post_title']) );
				echo '<br />';
				...
				if ( is_wp_error( $post_id ) ) {
					printf( esc_html__( 'Failed to import %1$s &#8220;%2$s&#8221;', 'wordpress-importer' ), $post_type_object->labels->singular_name, esc_html($post['post_title']) );
					if ( defined('IMPORT_DEBUG') && IMPORT_DEBUG )
