...
function aqualuxe_wishlist_button() {
    ...
    if ( is_user_logged_in() ) {
        ...
        echo '<a href="#" class="wishlist-toggle ' . ( $in_wishlist ? 'in-wishlist' : '' ) . '" data-product-id="' . esc_attr( $product->get_id() ) . '">';
        ...
        echo '<a href="' . esc_url( wc_get_page_permalink( 'myaccount' ) ) . '" class="wishlist-login-required" title="' . esc_attr__( 'Login to add to wishlist', 'aqualuxe' ) . '">';
        echo '<svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
        </svg>';
        echo '</a>';
...
add_action( 'woocommerce_after_shop_loop_item', 'aqualuxe_wishlist_button', 7 );
...
function aqualuxe_wishlist_ajax() {
    if ( ! is_user_logged_in() ) {
        wp_send_json_error( array( 'message' => __( 'You must be logged in to use the wishlist', 'aqualuxe' ) ) );
        die();
    }
    ...
    if ( isset( $_POST['product_id'] ) ) {
        ...
        if ( $in_wishlist ) {
            ...
            wp_send_json_success( array( 'action' => 'removed', 'message' => __( 'Product removed from wishlist', 'aqualuxe' ) ) );
        } else {
            // Add to wishlist
            $wishlist[] = $product_id;
            update_user_meta( $user_id, 'aqualuxe_wishlist', $wishlist );
            wp_send_json_success( array( 'action' => 'added', 'message' => __( 'Product added to wishlist', 'aqualuxe' ) ) );
        }
    }
...
function aqualuxe_currency_switcher() {
    // This is a placeholder for multi-currency support
    // Actual implementation would depend on the currency plugin used
    $currencies = array(
        'USD' => '$',
        'EUR' => '€',
        'GBP' => '£',
...
function aqualuxe_compare_button() {
    ...
    echo '<a href="#" class="compare-button" data-product-id="' . esc_attr( $product->get_id() ) . '">' . esc_html__( 'Compare', 'aqualuxe' ) . '</a>';
}
add_action( 'woocommerce_after_shop_loop_item', 'aqualuxe_compare_button', 9 );

/**
 * Add AJAX handler for product comparison
 */
function aqualuxe_compare_ajax() {
    if ( isset( $_POST['product_id'] ) ) {
        ...
        if ( ! isset( $_SESSION['aqualuxe_compare_products'] ) ) {
            $_SESSION['aqualuxe_compare_products'] = array();
        }
        
        $compare_products = $_SESSION['aqualuxe_compare_products'];
        
        if ( in_array( $product_id, $compare_products ) ) {
            // Remove from comparison
            $compare_products = array_diff( $compare_products, array( $product_id ) );
            $_SESSION['aqualuxe_compare_products'] = $compare_products;
            wp_send_json_success( array( 'action' => 'removed', 'message' => __( 'Product removed from comparison', 'aqualuxe' ) ) );
        } else {
            // Add to comparison (limit to 4 products)
            if ( count( $compare_products ) >= 4 ) {
                wp_send_json_error( array( 'message' => __( 'You can compare up to 4 products at a time', 'aqualuxe' ) ) );
            } else {
                $compare_products[] = $product_id;
                $_SESSION['aqualuxe_compare_products'] = $compare_products;
                wp_send_json_success( array( 'action' => 'added', 'message' => __( 'Product added to comparison', 'aqualuxe' ) ) );
            }
        }
    }
