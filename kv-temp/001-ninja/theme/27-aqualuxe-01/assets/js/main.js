...
(function($) {
  ...
  const aqualuxe = {
    ...
    darkMode: function() {
      const { darkModeToggle, body } = this.dom;
      ...
      if (savedTheme === 'dark' || (!savedTheme && systemDarkMode)) {
        body.addClass('dark');
        darkModeToggle.attr('aria-checked', 'true');
      } else {
        body.removeClass('dark');
        darkModeToggle.attr('aria-checked', 'false');
      }
      ...
      window.matchMedia('(prefers-color-scheme: dark)').addEventListener('change', e => {
        if (localStorage.getItem('aqualuxe-theme')) return; // Don't override user preference
    ...
    enhanceWooCommerce: function() {
      const { productGallery, quantityInputs } = this.dom;
      ...
      $(document).on('click', '.quick-view-button', function(e) {
        ...
        // Show loading state
        aqualuxe.dom.body.addClass('loading');
        ...
        $.ajax({
          ...
          success: function(response) {
            ...
            modal.html(response);
            aqualuxe.dom.body.append(modal);
            ...
            setTimeout(function() {
              modal.addClass('open');
              aqualuxe.dom.body.removeClass('loading');
            }, 100);
          ...
          error: function() {
            aqualuxe.dom.body.removeClass('loading');
            alert('Error loading product quick view.');
          }
        });
      });
    },
    ...
    ajaxFilters: function() {
      ...
      $('.post-filters').on('submit', function(e) {
        ...
        $.ajax({
          url: aqualuxeSettings.ajaxUrl,
          type: 'POST',
          data: formData + '&action=aqualuxe_filter_posts',
          success: function(response) {
