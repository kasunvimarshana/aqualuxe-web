...
// Export info section [aqualuxe_export_info]
if (\function_exists('add_shortcode')) { \add_shortcode('aqualuxe_export_info', function(){
        $contactUrl = (\function_exists('home_url') ? \call_user_func('home_url', '/contact') : '/contact');
        $wholesaleUrl = (\function_exists('home_url') ? \call_user_func('home_url', '/wholesale') : '/wholesale');
        $contactUrl = (\function_exists('esc_url') ? \call_user_func('esc_url', $contactUrl) : $contactUrl);
        $wholesaleUrl = (\function_exists('esc_url') ? \call_user_func('esc_url', $wholesaleUrl) : $wholesaleUrl);
        $t = function($s){ return (\function_exists('esc_html__') ? \call_user_func('esc_html__', $s, 'aqualuxe') : $s); };
        ob_start(); ?>
            <section class="prose dark:prose-invert max-w-none">
                <h2><?php echo $t('Global Export Services'); ?></h2>
                <p><?php echo $t('We export healthy, quarantined ornamental fish and aquatic plants worldwide with strict compliance and animal welfare standards.'); ?></p>
                <h3><?php echo $t('What we provide'); ?></h3>
                <ul>
                    <li><?php echo $t('Health checks and pre‑export quarantine'); ?></li>
                    <li><?php echo $t('Custom packing (oxygenated bags, temperature control, labeling)'); ?></li>
                    <li><?php echo $t('Documentation (invoices, packing lists, certificates where required)'); ?></li>
                    <li><?php echo $t('Logistics coordination to major destinations'); ?></li>
                </ul>
                <h3><?php echo $t('How it works'); ?></h3>
                <ol>
                    <li><?php echo $t('Request a quote with desired species, quantities, and destination.'); ?></li>
                    <li><?php echo $t('We confirm availability, estimates, and lead times.'); ?></li>
                    <li><?php echo $t('We prepare, quarantine, and pack according to best practices.'); ?></li>
                    <li><?php echo $t('We ship and share tracking and export documentation.'); ?></li>
                </ol>
                <div class="mt-6 flex gap-3">
                    <a class="btn btn-primary" href="<?php echo $contactUrl; ?>"><?php echo $t('Request a Quote'); ?></a>
                    <a class="btn btn-secondary" href="<?php echo $wholesaleUrl; ?>"><?php echo $t('Apply for Wholesale'); ?></a>
                </div>
            </section>
        <?php return (string) ob_get_clean();
}); }

// Export FAQ section [aqualuxe_export_faq]
if (\function_exists('add_shortcode')) { \add_shortcode('aqualuxe_export_faq', function(){
    $t = function($s){ return (\function_exists('esc_html__') ? \call_user_func('esc_html__', $s, 'aqualuxe') : $s); };
    $faqs = [
        [ 'q' => 'Which countries do you ship to?', 'a' => 'We ship to most regions. Some destinations require permits or prior approvals—contact us with your location.' ],
        [ 'q' => 'Do I need import permits or health certificates?', 'a' => 'Depending on your country, you may need permits and certificates. We can guide you and provide documents where required.' ],
        [ 'q' => 'What are the minimum order quantities (MOQ)?', 'a' => 'MOQs vary by species and route. Typical MOQs start around 20–30 bags per shipment. We will confirm per quote.' ],
        [ 'q' => 'How do you pack live fish for export?', 'a' => 'Oxygenated bags with species-appropriate water parameters, insulation, and temperature control. Boxes are clearly labeled.' ],
        [ 'q' => 'What are your payment terms?', 'a' => 'Bank transfer in advance unless otherwise agreed. For wholesale accounts we may offer additional options.' ],
        [ 'q' => 'What if there are DOA (dead on arrival)?', 'a' => 'Notify us within the agreed window with photos and airway bill. Approved DOA claims are credited on future orders as per policy.' ],
        [ 'q' => 'What are the lead times?', 'a' => 'Typically 7–14 days depending on availability, quarantine requirements, and flight schedules.' ],
    ];
    ...
    $json = function_exists('wp_json_encode') ? \call_user_func('wp_json_encode', $ld) : json_encode($ld);
    ob_start(); ?>
        <section class="mt-10">
            <h2 class="text-2xl font-semibold mb-4"><?php echo $t('Export FAQ'); ?></h2>
            <div class="space-y-3">
                <?php foreach ($faqs as $i => $f): $qid = 'aqlx-faq-' . ($i+1); ?>
                    <details class="border rounded p-3 bg-white/60 dark:bg-slate-900/60" itemscope itemprop="mainEntity" itemtype="https://schema.org/Question">
                        <summary class="cursor-pointer font-medium" id="<?php echo function_exists('esc_attr') ? call_user_func('esc_attr', $qid) : $qid; ?>" itemprop="name"><?php echo function_exists('esc_html') ? call_user_func('esc_html', $t($f['q'])) : $t($f['q']); ?></summary>
                        <div class="mt-2" itemscope itemprop="acceptedAnswer" itemtype="https://schema.org/Answer">
                            <p itemprop="text" class="prose dark:prose-invert max-w-none"><?php echo function_exists('esc_html') ? call_user_func('esc_html', $t($f['a'])) : $t($f['a']); ?></p>
                        </div>
                    </details>
                <?php endforeach; ?>
            </div>
