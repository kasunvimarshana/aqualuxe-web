...
<article id="frontpage" class="ocean-gradient text-slate-900 dark:text-slate-100">
	...
	<section class="relative min-h-[72vh] flex items-end pb-12" aria-labelledby="hero-title">
		...
		<!-- Dynamic SVG waves (animated via JS) -->
		...
		<div class="container relative z-10 max-w-7xl mx-auto px-4 grid md:grid-cols-2 gap-8 items-end">
			<div class="py-10">
				<h1 id="hero-title" class="text-4xl md:text-6xl font-extrabold drop-shadow leading-tight">
					Dive into a next‑generation aquatic experience.
				</h1>
				<p class="mt-4 max-w-xl opacity-90">
					Smooth, captivating animations flow like water—crafted to engage and convert with every click. AquaLuxe blends elegant storytelling with seamless interactivity in harmony with its aquatic spirit.
				</p>
				<div class="mt-6 flex flex-wrap gap-3">
					<?php
					// Resolve shop link without Woo functions
					$shop_link = '#';
					$sid = absint(get_option('woocommerce_shop_page_id'));
					if ($sid > 0) { $shop_link = get_permalink($sid); }
					if ($shop_link === '#') {
					...
					?>
					<a href="<?php echo esc_url($shop_link); ?>" class="px-6 py-3 rounded bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900 hover-lift">Shop Now</a>
					<a href="#discover" class="px-6 py-3 rounded border hover-lift">Discover</a>
					<button id="aqlx-audio-toggle" class="px-6 py-3 rounded border hover-lift" aria-pressed="false" aria-label="Toggle ambient ocean audio">Audio: Off</button>
				</div>
	...
	<section id="discover" class="container max-w-7xl mx-auto px-4 py-14" data-reveal>
		<div class="grid md:grid-cols-3 gap-6">
			<article class="p-5 border rounded hover-lift">
				<h3 class="font-semibold">Wellbeing & Quarantine</h3>
				<p class="opacity-80 text-sm mt-2">Export‑ready stock conditioned under best‑practice protocols for health and welfare.</p>
			</article>
			<article class="p-5 border rounded hover-lift">
				<h3 class="font-semibold">Aquascape Design</h3>
				<p class="opacity-80 text-sm mt-2">From nano tanks to commercial walls—design, build, and maintain with signature elegance.</p>
			</article>
			<article class="p-5 border rounded hover-lift">
				<h3 class="font-semibold">Sustainable Sourcing</h3>
				<p class="opacity-80 text-sm mt-2">Traceable breeder networks and habitat‑friendly practices for long‑term stewardship.</p>
			</article>
		</div>
	</section>

	<!-- METRICS: multi‑series D3 chart with toggles -->
	<section class="bg-white/70 dark:bg-slate-900/40 py-12" aria-labelledby="metrics-title" data-reveal>
		<div class="container max-w-7xl mx-auto px-4">
			...
			<p class="opacity-80 mb-6">Live or sample metrics—temperature, pH, ammonia—rendered with accessible, colorblind‑safe visuals.</p>
	...
	<section class="bg-white/70 dark:bg-slate-900/40 py-12" aria-labelledby="reco-title" data-reveal>
		<div class="container max-w-7xl mx-auto px-4">
			...
			<div id="aqlx-recos" class="mt-6 grid sm:grid-cols-2 md:grid-cols-3 gap-6" aria-live="polite" aria-busy="false">
				<?php
				// Server fallback items (replaced by JS hydration)
				$items = [];
				...
				if ($items) {
					foreach ($items as $post_id) {
						...
						$title = get_the_title($post); $link = get_permalink($post);
						...
						if (has_post_thumbnail($post)) {
							$thumb_html = get_the_post_thumbnail($post, 'medium', [
								...
								'loading' => 'lazy', 'decoding' => 'async', 'alt' => esc_attr($title),
							]);
						}
						$price_html = '';
	...
	<section class="container max-w-7xl mx-auto px-4 py-14" data-reveal>
		<div class="rounded-xl border p-6 md:p-8 flex flex-col md:flex-row items-center justify-between gap-4 bg-white/70 dark:bg-slate-900/40">
			<div>
				<h2 class="text-xl font-semibold">Join the Current</h2>
				<p class="opacity-80 text-sm mt-1">Get product drops, aquascape tips, and export windows.</p>
			</div>
			<form class="flex w-full md:w-auto gap-2" action="#" method="post" novalidate>
				...
				<button type="submit" class="px-5 py-2 rounded bg-slate-900 text-white dark:bg-slate-100 dark:text-slate-900 hover-lift">Subscribe</button>
			</form>
		</div>
	</section>
