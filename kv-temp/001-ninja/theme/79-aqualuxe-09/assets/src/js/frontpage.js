...
function mountWaves() {
  if (prefersReducedMotion) return;
  const svg = document.getElementById('aqlx-waves');
  if (!svg) return;
  const paths = svg.querySelectorAll('path');
  if (!paths.length) return;
  import(/* webpackChunkName: "gsap" */ 'gsap').then(g => {
    const gsap = g.gsap || g.default || g;
    paths.forEach((p, i) => {
      const amp = 6 + i * 3;
      const dur = 4 + i * 1.5;
      gsap.to(p, { y: i % 2 === 0 ? -amp : amp, duration: dur, ease: 'sine.inOut', yoyo: true, repeat: -1 });
      gsap.to(p, { x: i % 2 === 0 ? 8 : -8, duration: dur * 1.2, ease: 'sine.inOut', yoyo: true, repeat: -1 });
    });
  }).catch(() => {});
}
