...
(function(){
  ...
  function loop(){
    if (!running) return;
    spawn();
    const base = lowPower ? 480 : 200;
    const jitter = lowPower ? 420 : 400;
    setTimeout(loop, base + Math.random()*jitter);
  }
  loop();
  document.addEventListener('visibilitychange', ()=>{ running = !document.hidden; if (running) loop(); });

  // React to global low power events
  window.addEventListener('ax:lowpower', (e)=>{
    lowPower = !!(e?.detail?.on);
  });
