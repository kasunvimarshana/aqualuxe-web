...
function render() {
	...
	if ( isset( $_POST['_alx_nonce'] ) && ( $verify_nonce ? (bool) \call_user_func( $verify_nonce, $_POST['_alx_nonce'], 'alx_settings' ) : true ) ) {
        ...
        if ( $update_option ) { \call_user_func( $update_option, 'aqualuxe_sitemap_enabled', $sitemap_enabled ); }
		// Sitemap excluded post types (CSV)
		$excluded_types = isset($_POST['sitemap_excluded_types']) ? (string) $_POST['sitemap_excluded_types'] : '';
		if ( $update_option ) { \call_user_func( $update_option, 'aqualuxe_sitemap_excluded_types', sanitize_text_field( $excluded_types ) ); }
	// Sitemap images toggle
	$sitemap_images_enabled = isset( $_POST['sitemap_images_enabled'] ) ? '1' : '0';
	if ( $update_option ) { \call_user_func( $update_option, 'aqualuxe_sitemap_images_enabled', $sitemap_images_enabled ); }
	...
	$sitemap_enabled_val = (string) ( $get_option ? \call_user_func( $get_option, 'aqualuxe_sitemap_enabled', '1' ) : '1' );
	$sitemap_excluded_types_val = (string) ( $get_option ? \call_user_func( $get_option, 'aqualuxe_sitemap_excluded_types', '' ) : '' );
	$sitemap_images_enabled_val = (string) ( $get_option ? \call_user_func( $get_option, 'aqualuxe_sitemap_images_enabled', '1' ) : '1' );
	...
	<div class="wrap">
		...
		<form method="post">
			...
			<table class="form-table" role="presentation"><tbody>
				<tr><th><?php \esc_html_e( 'SEO', 'aqualuxe' ); ?></th>
				<td>
					<label><input type="checkbox" name="sitemap_enabled" value="1" <?php echo ( $sitemap_enabled_val === '1' ? 'checked' : '' ); ?> /> <?php \esc_html_e( 'Enable theme XML sitemap (auto-disables if another SEO plugin provides sitemaps).', 'aqualuxe' ); ?></label>
					<p style="margin-top:8px">
						<label><input type="checkbox" name="sitemap_images_enabled" value="1" <?php echo ( $sitemap_images_enabled_val === '1' ? 'checked' : '' ); ?> /> <?php \esc_html_e( 'Include images (featured, inline, attached) in sitemaps.', 'aqualuxe' ); ?></label>
					</p>
					<p style="margin-top:8px">
						<label><?php \esc_html_e('Excluded post types (CSV of slugs)','aqualuxe'); ?>
							<input type="text" class="regular-text" name="sitemap_excluded_types" value="<?php echo \esc_attr( $sitemap_excluded_types_val ); ?>" placeholder="product,alx_classified" />
						</label>
					</p>
				</td></tr>
