...
class SEO extends Service {
	...
	public function register() {
		// Only run SEO optimizations if enabled
		if ( ! get_theme_mod( 'aqualuxe_enable_seo_optimizations', true ) ) {
			return;
		}
		...
		add_action( 'wp_head', array( $this, 'add_twitter_card_tags' ), 5 );
		...
		add_filter( 'the_content', array( $this, 'add_rel_attributes' ) );
		
		// Add image alt text
		add_filter( 'the_content', array( $this, 'add_missing_alt_text' ) );
		...
		// Add meta keywords
		if ( get_theme_mod( 'aqualuxe_enable_meta_keywords', false ) ) {
			add_action( 'wp_head', array( $this, 'add_meta_keywords' ), 1 );
		}
		
		// Add next/prev links for pagination
		add_action( 'wp_head', array( $this, 'add_next_prev_links' ), 1 );
		
		// Add hreflang links for multilingual sites
		if ( get_theme_mod( 'aqualuxe_enable_hreflang', false ) ) {
			add_action( 'wp_head', array( $this, 'add_hreflang_links' ), 1 );
		}
		
		// Add social profile links to JSON-LD
		add_filter( 'aqualuxe_json_ld_social_profiles', array( $this, 'add_social_profiles' ) );
	}
	...
	public function add_json_ld_structured_data() {
		...
		$social_profiles = apply_filters( 'aqualuxe_json_ld_social_profiles', array() );
	 ...
	 */
	public function add_rel_attributes( $content ) {
	 ...
	 * @return string Modified content.
	 */
	public function add_missing_alt_text( $content ) {
	...
	/**
	 * Add social profiles to JSON-LD.
	 *
	 * @param array $profiles Social profiles.
	 * @return array Modified social profiles.
	 */
	public function add_social_profiles( $profiles ) {
