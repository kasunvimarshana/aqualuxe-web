					...
					if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
						...
						<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
							...
							if (function_exists('aqualuxe_get_estimated_delivery')) {
								$estimated_delivery = aqualuxe_get_estimated_delivery($product_id);
								if ($estimated_delivery) {
									echo '<div class="estimated-delivery text-xs text-gray-600 mt-2">';
									echo '<span class="delivery-label">' . esc_html__('Estimated Delivery:', 'aqualuxe') . '</span> ';
									echo '<span class="delivery-date">' . esc_html($estimated_delivery) . '</span>';
									echo '</div>';
								}
							}
							?>
					...
					<td colspan="6" class="actions">
						...
						<?php if ( wc_coupons_enabled() ) { ?>
							<div class="coupon">
								...
								<button type="submit" class="button<?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ? ' ' . wc_wp_theme_get_element_class_name( 'button' ) : '' ); ?>" name="apply_coupon" value="<?php esc_attr_e( 'Apply coupon', 'aqualuxe' ); ?>"><?php esc_attr_e( 'Apply coupon', 'aqualuxe' ); ?></button>
