...
function aqualuxe_generate_compatibility_pdf() {
    ...
    $html .= '
        ...
        <p>Tank Utilization: ' . round($data['spaceRequirements']['spaceUtilization']) . '%</p>
        <table>
            <tr>
                <th>Fish</th>
                <th>Minimum Group</th>
    ...
    if ($data['parameterCompatibility']['ph']['compatible']) {
        $html .= '<td>' . $data['parameterCompatibility']['ph']['min'] . ' - ' . $data['parameterCompatibility']['ph']['max'] . '</td>';
    ...
    if (!empty($data['recommendations'])) {
        ...
        foreach ($data['recommendations'] as $rec) {
            $html .= '<div class="recommendation ' . esc_attr($rec['priority']) . '">';
            $html .= '<p><strong>' . esc_html($rec['message']) . '</strong></p>';
            $html .= '<p>' . esc_html($rec['solution']) . '</p>';
            $html .= '</div>';
        }
    } else {
        $html .= '
    ...
    $filename = 'fish-compatibility-' . date('Y-m-d') . '.pdf';
    ...
    $temp_html_path = $upload_dir['path'] . '/temp-' . md5(time()) . '.html';
