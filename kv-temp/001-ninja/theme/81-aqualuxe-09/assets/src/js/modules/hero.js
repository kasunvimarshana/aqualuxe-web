...
(function(){
  ...
  canvas.addEventListener('pointerdown', ()=>{ spawnRipple(pointerWorld.clone()); }, { passive: true });
  ...
  const audioIcon = audioBtn ? audioBtn.querySelector('.ax-audio-ic') : null;
  ...
  if (audioBtn) {
    ...
    try {
      ...
      if (savedVol !== null) { const v = Math.max(0, Math.min(100, parseInt(savedVol,10)||0)); volume = (v/100)*0.2; if (volSlider) volSlider.value = String(v); }
      if (pref === 'on') { enableAudio(); audioBtn.setAttribute('aria-pressed','true'); audioBtn.setAttribute('aria-label','Disable ambient sound'); if (audioIcon) audioIcon.textContent = '🔊'; }
    } catch(e){}
    audioBtn.addEventListener('click', ()=>{
      if (!audioEnabled) { enableAudio(); audioBtn.setAttribute('aria-pressed','true'); audioBtn.setAttribute('aria-label','Disable ambient sound'); if (audioIcon) audioIcon.textContent = '🔊'; try{ localStorage.setItem('ax:audio','on'); }catch(e){} }
      else { disableAudio(); audioBtn.setAttribute('aria-pressed','false'); audioBtn.setAttribute('aria-label','Enable ambient sound'); if (audioIcon) audioIcon.textContent = '🔈'; try{ localStorage.setItem('ax:audio','off'); }catch(e){} }
    });
  }
