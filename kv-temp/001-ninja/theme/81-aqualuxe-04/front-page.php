...
<section class="ax-hero-section relative overflow-hidden" aria-label="Immersive hero section with animated ocean visuals" role="region">
  ...
  .ax-hero-overlay::after{content:"";position:absolute;left:50%;transform:translateX(-50%);top:10%;width:min(100%,1200px);height:clamp(220px,45vw,520px);z-index:-1;
      background:radial-gradient(ellipse at center, rgba(255,255,255,.75) 0%, rgba(255,255,255,.55) 35%, rgba(255,255,255,.15) 60%, rgba(255,255,255,0) 75%);
      filter:blur(24px);pointer-events:none}
    @media(prefers-color-scheme: dark){
      .ax-hero-overlay::after{background:radial-gradient(ellipse at center, rgba(15,23,42,.6) 0%, rgba(15,23,42,.45) 40%, rgba(15,23,42,0) 70%)}
    }
  ...
  @media(prefers-color-scheme: dark){.ax-hero-overlay h1,.ax-hero-overlay p{color:#e5e7eb}}
    ...
    @media(max-width:767px){
      .ax-hero-overlay{align-items:flex-start;padding-top:3.5rem}
