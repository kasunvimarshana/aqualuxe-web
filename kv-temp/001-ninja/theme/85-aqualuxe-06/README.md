...
- Built assets: `theme.css`, `tailwind.css`, `skin-dark.css`, `editor.css`, `theme.js` (all versioned via mix-manifest)
...
### Editor styles
- The block editor loads `editor.css` (compiled from Tailwind tokens) via `enqueue_block_editor_assets`.
- Theme declares `editor-styles`, `wp-block-styles`, and `align-wide` supports.
