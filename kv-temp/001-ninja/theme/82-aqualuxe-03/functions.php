...
add_action( 'wp_enqueue_scripts', function () {
	...
	if ( function_exists('wp_localize_script') ) {
		...
		$hero_sfx   = function_exists('get_theme_mod') ? (bool) get_theme_mod('aqualuxe_enable_hero_sfx', 1) : true;
...
// Classic widget areas (progressive enhancement)
...
// Initialize admin early so its admin_menu hook is registered before WordPress fires admin_menu.
add_action( 'init', function(){
...
add_action( 'after_setup_theme', function(){
	...
	$default_modules = [ 'dark-mode', 'multilingual', 'services', 'events', 'woocommerce', 'wishlist', 'quick-view', 'filtering', 'currency', 'classifieds', 'roles', 'themes', 'sitemap', 'importer' ];
