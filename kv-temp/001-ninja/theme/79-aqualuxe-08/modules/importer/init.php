...
function render(): void
{
  ...
  <div class="wrap">
    ...
    <div style="display:flex;gap:8px;flex-wrap:wrap;align-items:center;">
      ...
      <label><input type="checkbox" id="aqlx-rollback" checked> <?php esc_html_e('Rollback on error', 'aqualuxe'); ?></label>
          <label><input type="checkbox" id="aqlx-dryrun"> <?php esc_html_e('Dry run (no changes)', 'aqualuxe'); ?></label>
    </div>
    <div style="margin-top:12px;display:flex;gap:8px;flex-wrap:wrap;">
      ...
      <button class="button" id="aqlx-preview"><?php esc_html_e('Preview', 'aqualuxe'); ?></button>
          ...
          <button class="button" id="aqlx-pause" disabled><?php esc_html_e('Pause', 'aqualuxe'); ?></button>
          <button class="button" id="aqlx-resume" disabled><?php esc_html_e('Resume', 'aqualuxe'); ?></button>
          <button class="button button-link" id="aqlx-cancel" disabled><?php esc_html_e('Cancel', 'aqualuxe'); ?></button>
  ...
  let busy = false;
    ...
    return new URLSearchParams({
      ...
      dryrun: document.getElementById('aqlx-dryrun').checked ? '1':'0',
      ...extra
    });
    }
    ...
    function poll(){
        ajax('aqlx_demo_status').then(r=>{
          if(!r||!r.success) return;
          const d=r.data;
          statusEl.textContent = d.message||d.status;
          progEl.value = d.progress||0;
      const running = (d && d.status==='running');
      nextBtn.disabled = !running;
      document.getElementById('aqlx-pause').disabled = !running || !autoRun;
      document.getElementById('aqlx-resume').disabled = !running || autoRun;
      document.getElementById('aqlx-cancel').disabled = !running;
          if(d.log){ logEl.textContent=''; d.log.forEach(line=>log(line)); }
          // Auto-advance steps while job is running
      if(running && autoRun && !busy){
            busy = true;
            ajax('aqlx_demo_next_step').then(()=>{ busy=false; }).catch(()=>{ busy=false; });
          }
        });
      }
    setInterval(poll, 1200);
    document.getElementById('aqlx-start').addEventListener('click', function(){ log('Starting job...'); ajax('aqlx_demo_start').then(r=>{ if(!r.success) { log('ERR '+(r.data||'fail')); } else { log('Job started'); autoRun = true; } }).catch(e=>log('ERR '+e)); });
    ...
    document.getElementById('aqlx-pause').addEventListener('click', function(){ autoRun = false; log('Auto-run paused'); });
    document.getElementById('aqlx-resume').addEventListener('click', function(){ autoRun = true; log('Auto-run resumed'); });
    document.getElementById('aqlx-cancel').addEventListener('click', function(){ autoRun = false; ajax('aqlx_demo_cancel').then(r=>{ if(r&&r.success){ log('Job canceled'); } else { log('Cancel failed'); } }); });
...
add_action('wp_ajax_aqlx_demo_cancel', __NAMESPACE__ . '\\ajax_demo_cancel');
...
function ajax_demo_cancel(): void { verify(); $job = get_job(); $job['status'] = 'canceled'; save_job($job); log_msg('Job canceled by user.'); wp_send_json_success(['ok'=>true]); }
...
function run_next_step(): array {
  ...
  $params = $job['params'] ?? [];
  try {
    $dry = !empty($params['dryrun']);
    switch ($step) {
      case 0:
        if ($dry) { log_msg('DRY RUN: would ensure roles and CPTs.'); } else { ensure_roles(); ensure_cpts(); }
        log_msg('Environment ensured.');
        break;
      case 1:
        if (in_array('users', $params['entities'], true)) { if ($dry) { log_msg('DRY RUN: would create users.'); } else { create_users($job, $params); } }
        break;
      case 2:
        if ($params['media']) { if ($dry) { log_msg('DRY RUN: would generate media.'); } else { create_media($job, $params); } }
        break;
      case 3:
        if (in_array('pages', $params['entities'], true)) { if ($dry) { log_msg('DRY RUN: would create pages and assign front/blog.'); } else { create_pages($job, $params); } }
        break;
      case 4:
        if (in_array('posts', $params['entities'], true)) { if ($dry) { log_msg('DRY RUN: would create posts.'); } else { create_posts($job, $params); } }
        break;
      case 5:
        if (class_exists('WooCommerce') && in_array('products', $params['entities'], true)) { if ($dry) { log_msg('DRY RUN: would create products, attributes, variations, and Woo pages.'); } else { create_products($job, $params); } }
        break;
      case 6:
        if (in_array('menus', $params['entities'], true)) { if ($dry) { log_msg('DRY RUN: would setup menus and assign locations.'); } else { setup_menus($job, $params); } }
        break;
      case 7:
        if (in_array('widgets', $params['entities'], true)) { if ($dry) { log_msg('DRY RUN: would add widgets.'); } else { setup_widgets($job, $params); } }
        break;
      case 8:
        if (in_array('services', $params['entities'], true) || in_array('events', $params['entities'], true)) { if ($dry) { log_msg('DRY RUN: would create services/events.'); } else { create_services_events($job, $params); } }
        break;
      case 9:
        if ($dry) { log_msg('DRY RUN: would finalize site (flush caches, etc).'); } else { finalize_site($job, $params); }
        log_msg('Finalize done.');
        break;
    }
    $step++;
    $job['step'] = $step;
...
function create_users(array &$job, array $p): void {
  ...
  if (! is_wp_error($uid) && $uid) { add_user_meta((int)$uid, META_KEY, 1, true); $job['created']['users'][] = (int) $uid; }
  }
}
...
function create_products(array &$job, array $p): void {
  ...
  for ($i=0;$i<$count;$i++) {
    ...
    if (! is_wp_error($pid)) {
      ...
      // Assign placeholder image if media enabled
      if (! empty($p['media'])) { $aid = create_placeholder_image($title); if ($aid) { set_post_thumbnail($pid, $aid); } }
  ...
  if ($var_id) {
    ...
    if (! empty($p['media'])) { $aid = create_placeholder_image($var_title); if ($aid) { set_post_thumbnail($var_id, $aid); } }
...
function hard_flush(): void
{
  ...
  // Only remove users flagged as demo
  $demo_users = get_users(['meta_key'=>META_KEY,'meta_value'=>1,'fields'=>'ids']);
  foreach ($demo_users as $uid) { wp_delete_user((int) $uid); }
}
