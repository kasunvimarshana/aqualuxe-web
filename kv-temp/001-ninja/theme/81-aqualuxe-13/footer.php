...
(function(){
  document.addEventListener('click', function(e){
    const el = e.target.closest('[data-ax-event]');
    if (!el) return; const evt = el.getAttribute('data-ax-event');
    try { localStorage.setItem('ax:last_event', evt + ':' + Date.now()); } catch(e){}
    if (window.dataLayer && Array.isArray(window.dataLayer)) {
      window.dataLayer.push({ event:'aqualuxe_cta', action:evt });
    }
  });
})();
</script>
