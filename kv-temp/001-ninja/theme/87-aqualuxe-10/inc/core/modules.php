    ...
    $db = \function_exists('get_option') ? (array) \call_user_func('get_option', 'aqualuxe_modules', []) : [];
        ...
        foreach ($config as $slug => $enabled) {
            ...
            if (file_exists($file)) {
                try {
                    require_once $file;
                    // Optional: class-based module support (non-breaking).
                    $class = 'AquaLuxe\\Modules\\' . self::studly($slug);
                    if (class_exists($class)) {
                        $instance = null;
                        try { $instance = new $class(); } catch (\Throwable $e) { /* ignore */ }
                        if ($instance && is_object($instance) && method_exists($instance, 'boot')) {
                            try { $instance->boot(); } catch (\Throwable $e) {
                                $logger = \AquaLuxe\Core\Container::get('logger');
                                if ($logger && method_exists($logger, 'error')) {
                                    $logger->error('Module {slug} boot error: {msg}', ['slug' => $slug, 'msg' => $e->getMessage()]);
                                }
                            }
                        }
                    }
