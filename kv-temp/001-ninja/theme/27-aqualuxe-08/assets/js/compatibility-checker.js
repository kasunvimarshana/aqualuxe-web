    ...
    const CompatibilityChecker = {
        ...
        searchFish: function(query) {
            ...
            limitedResults.forEach(fish => {
                ...
                if (fish.image) {
                    ...
                    const imgPath = fish.image || aqualuxe_compatibility.default_fish_image;
                    resultsHtml += `<div class="fish-result-image"><img src="${imgPath}" alt="${fish.name}" onerror="this.src='${aqualuxe_compatibility.default_fish_image}'"></div>`;
                } else {
                    // Use default image if no image is provided
                    resultsHtml += `<div class="fish-result-image"><img src="${aqualuxe_compatibility.default_fish_image}" alt="${fish.name}"></div>`;
                }
        ...
        addFish: function(fishId) {
            ...
            if (fish.image) {
                // Check if the image path exists and is valid
                const imgPath = fish.image || aqualuxe_compatibility.default_fish_image;
                fishHtml += `<div class="selected-fish-image"><img src="${imgPath}" alt="${fish.name}" onerror="this.src='${aqualuxe_compatibility.default_fish_image}'"></div>`;
            } else {
                // Use default image if no image is provided
                fishHtml += `<div class="selected-fish-image"><img src="${aqualuxe_compatibility.default_fish_image}" alt="${fish.name}"></div>`;
            }
        ...
        saveResults: function() {
            ...
            $.ajax({
                ...
                error: function(xhr, status, error) {
                    ...
                    alert('Error connecting to server. Please try again. ' + (xhr.responseJSON && xhr.responseJSON.data ? xhr.responseJSON.data.message : error));
                }
            });
        }
    };
