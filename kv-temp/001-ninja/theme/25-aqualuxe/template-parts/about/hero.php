...
$hero_title = get_post_meta($page_id, '_aqualuxe_about_hero_title', true);
if (!$hero_title) {
    $hero_title = get_theme_mod('aqualuxe_about_hero_title', __('About AquaLuxe', 'aqualuxe'));
}

$hero_subtitle = get_post_meta($page_id, '_aqualuxe_about_hero_subtitle', true);
if (!$hero_subtitle) {
    $hero_subtitle = get_theme_mod('aqualuxe_about_hero_subtitle', __('Our Journey & Mission', 'aqualuxe'));
}

$hero_text = get_post_meta($page_id, '_aqualuxe_about_hero_text', true);
if (!$hero_text) {
    $hero_text = get_theme_mod('aqualuxe_about_hero_text', __('Learn about our story, our team, and our commitment to sustainable ornamental fish farming.', 'aqualuxe'));
}

$hero_image = get_post_meta($page_id, '_aqualuxe_about_hero_image', true);
if (!$hero_image) {
    $hero_image = get_theme_mod('aqualuxe_about_hero_image', get_template_directory_uri() . '/assets/images/about-hero.jpg');
}

$hero_overlay = get_post_meta($page_id, '_aqualuxe_about_hero_overlay', true);
if ($hero_overlay === '') {
...
<section class="about-hero-section relative min-h-[50vh] <?php echo esc_attr($overlay_class); ?> flex items-center">
    <?php if ($hero_image) : ?>
